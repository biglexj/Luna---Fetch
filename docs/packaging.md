...
El flujo ejecuta pruebas de escritorio, compila Android, prepara EXE/MSI, copia tres APK de depuración por arquitectura y calcula `SHA256SUMS.txt`. En Linux genera `.pkg.tar.zst`, `.AppImage` y `.deb`. Los artefactos quedan en `release/`.
...
Para Linux se utiliza el empaquetador `./scripts/build/build-linux.sh` y la receta [packaging/arch/PKGBUILD](../packaging/arch/PKGBUILD).
